...
// ==================== contracts/YieldFarmingAggregator.sol ====================
...
contract YieldFarmingAggregator is Ownable, ReentrancyGuard, Pausable {
    ...
    struct Strategy {
        uint256[] poolIds;
        uint256[] allocations; // percentage allocation to each pool (basis points)
    ...
    /**
     * @dev Core Function 1: Stake tokens into a specific pool with optimal allocation
    ...
    /**
     * @dev Core Function 2: Execute automated yield optimization strategy
     * @param strategyId The ID of the strategy to execute
     * @param totalAmount The total amount to distribute across pools
     */
    function executeStrategy(uint256 strategyId, uint256 totalAmount) external nonReentrant whenNotPaused {
        ...
        // Calculate and distribute amounts across pools
    ...
    /**
     * @dev Core Function 3: Withdraw tokens and claim all pending rewards
    ...
    function withdrawAndClaim(uint256 poolId, uint256 amount) external nonReentrant {
        ...
        // If amount is 0, withdraw all staked tokens
        if (amount == 0) {
        ...
        // Update pool and harvest rewards
        _updatePool(poolId);
    ...
    function _harvestRewards(uint256 poolId, address userAddress) internal returns (uint256) {
        ...
        if (totalRewards > 0) {
            ...
            // Transfer reward tokens (assuming same token for simplicity)
    ...
    function emergencyWithdraw(uint256 poolId) external nonReentrant {
        ...
        // Transfer tokens back (no rewards in emergency)
        IERC20(pool.stakingToken).transfer(msg.sender, amount);
    ...
    function getUserInfo(uint256 poolId, address userAddress) external view returns (
        uint256 stakedAmount,
        uint256 pendingRewards,
        uint256 totalRewardsEarned,
        uint256 lastStakeTime
